...
# MC
def pred_MC(model, datatype, q, num_workers, folder, format_name):
    df_test  = pd.read_hdf(dataset, key='MC_valid')
    df_test_prime = df_test.drop(['label'], axis=1)
    prd = tu.predict(model,
                  df_test_prime,
                  datatype,
                  batch_size=1024,
...
def pred_data(model, datatype, q, num_workers, folder, format_name, runs=[7470, 7471, 7472, 7473]):
...
if __name__ == '__main__':
    ...
    if action == 'predict':
        ...
        pred_MC(model, data_type, 4, folder, format_name)
        pred_data(model, data_type, 4, folder, format_name, runs=[7470, 7471, 7472, 7473])
